...
def set_path(request: HttpRequest):
    if request.method == "POST":
        ...
        if response is None:
            ...
            return HttpResponse(response.get_body())
...
def next_folder(request: HttpRequest):
    if request.method == "POST":
        request.session["path"] = str(
            NextFolder.next_folder(HexConversions.from_hex_to_string(request.headers["Folder-Name"]),
...
def download_file(request: HttpRequest):
    if request.method == "GET":
        ...
        response = HttpResponse(
            DownloadFile.download_file(request.session["login"], request.session["password"], file_name,
                                       request.session["path"]),
            content_type="application/octet-stream"
        )

        response["Content-Disposition"] = "inline; filename=" + translit(file_name, "ru", reversed=True)
